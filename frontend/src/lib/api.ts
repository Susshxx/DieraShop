...
/**
 * Build a ws:// or wss:// URL against the same host as the API, for the
 * realtime product-change socket. Note: in dev, Vite's /api proxy does NOT
 * proxy WebSocket upgrades by default, so this connects straight to the
 * Express backend's own port (OAUTH_BASE), not through the Vite dev server.
 */
export function wsUrl(path: string): string {
  const base = OAUTH_BASE.replace(/^http/, "ws"); // http -> ws, https -> wss
  return `${base}${path}`;
}
