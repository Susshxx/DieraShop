...
const AlertDialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  // Subtle top divider using theme border color
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-4 mt-2 border-t border-primary/15",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description
    ref={ref}
    // Soft accent background behind description text
    className={cn(
      "text-sm text-muted-foreground rounded-lg bg-accent/40 px-4 py-3 border border-primary/10",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  // Uses existing primary button style — already theme-aware
  <AlertDialogPrimitive.Action
    ref={ref}
    className={cn(buttonVariants(), "shadow-sm", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  // Outline cancel button with soft primary tint on border
  <AlertDialogPrimitive.Cancel
    ref={ref}
    className={cn(
      buttonVariants({ variant: "outline" }),
      "mt-2 sm:mt-0 border-primary/30 hover:bg-accent/60 hover:border-primary/50",
      className,
    )}
    {...props}
  />
